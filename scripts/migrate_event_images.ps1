<#
.SYNOPSIS
Copies image files listed in the Event Management IMAGES_TO_COPY.txt from the repository `/.attachments/` folder into `assets/app-starter-kits/event-management/`.

.DESCRIPTION
The script reads `IMAGES_TO_COPY.txt` in the event-management assets folder and copies every referenced image file from `.attachments` into the target folder. It reports progress and missing files and returns a non-zero exit code if any images were not found.

.PARAMETER SourceDir
Optional path to the source attachments directory. Defaults to the repository `.attachments` folder.

.PARAMETER TargetDir
Optional path to the destination folder. Defaults to `assets/app-starter-kits/event-management` inside the repo.

.PARAMETER ListFile
Optional path to the images manifest file. Defaults to the IMAGES_TO_COPY.txt in the event-management assets folder.
...
.")
# Note: To run: .\scripts\migrate_event_images.ps1 -DryRun
...
if (-not $TargetDir) {
    $TargetDir = Join-Path $repoRoot "assets\app-starter-kits\event-management"
}
if (-not $ListFile) {
