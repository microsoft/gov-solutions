<#
.SYNOPSIS
Copies image files listed in the Executive Coordination IMAGES_TO_COPY.txt from the repository `/.attachments/` folder into `assets/app-starter-kits/executive-coordination/`.

.DESCRIPTION
The script reads `IMAGES_TO_COPY.txt` in the executive-coordination assets folder and copies every referenced image file from `.attachments` into the target folder. It reports progress and missing files and returns a non-zero exit code if any images were not found.

.PARAMETER SourceDir
Optional path to the source attachments directory. Defaults to the repository `.attachments` folder.

.PARAMETER TargetDir
Optional path to the destination folder. Defaults to `assets/app-starter-kits/executive-coordination` inside the repo.

.PARAMETER ListFile
Optional path to the images manifest file. Defaults to the IMAGES_TO_COPY.txt in the executive-coordination assets folder.
...
# .\scripts\migrate_executive_coordination_images.ps1 -DryRun
#>
param(
    [string]$SourceDir,
    [string]$TargetDir,
...
if (-not $TargetDir) {
    $TargetDir = Join-Path $repoRoot "assets\app-starter-kits\executive-coordination"
}
if (-not $ListFile) {
...
foreach ($line in $lines) {
    ...
    # If the line starts with - or * strip that and trim again
    ...
    # If the candidate looks like a filename, accept it. Otherwise try to extract a filename by regex
