<#
.SYNOPSIS
Copies image files listed in the HR Administration IMAGES_TO_COPY.txt from the repository `/.attachments/` folder into `assets/app-starter-kits/hr-administration/`.
#>
param(
    [string]$SourceDir,
    [string]$TargetDir,
...
if (-not $TargetDir) { $TargetDir = Join-Path $repoRoot "assets\app-starter-kits\hr-administration" }
